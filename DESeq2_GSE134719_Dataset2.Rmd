...
#label samples
#Plot PCA function in viridis colors
...
#Gene symbol to rownames
conv_cts19 <- tibble:::column_to_rownames(conv_cts19, "gene_name")

#Find out WT vs PDK2KO for labeling
rld_19$Genotype
cts_19$names
#label column names for experimental vs control
colnames(conv_cts19) <- c("Pkd2KO", "Pkd2KO", "Pkd2KO", "wildtype", "wildtype", "wildtype", "wildtype", "Pkd2KO", "Pkd2KO", "Pkd2KO", "Pkd2KO", "wildtype", "wildtype", "wildtype", "wildtype", "Pkd2KO", "Pkkd2KO", "Pkd2KO", "Pkd2KO", "wildtype", "wildtype", "wildtype", "wildtype")
...
#top20 genes Post Cyst P28
fixedtop20_post_cyst_P28 <- order(conv_cts_fixed19$Pkd2KO, decreasing = TRUE)[1:20]
#fixedtop20test <- order(conv_cts_fixed, decreasing= TRUE)[1:20]
#Create heatmap w/ viridis colors
pheatmap(conv_cts_fixed19[fixedtop20_post_cyst_P28, ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = TRUE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Top 20 Positive Genes Cystic P28", angle_col = 45)

#bottom 20 genes w/ conv_cts
fixedbottom20_post_cyst_P21 <- order(conv_cts_fixed19$Pkd2KO, decreasing = FALSE)[1:20]
pheatmap(conv_cts_fixed19[fixedbottom20_post_cyst_P21, ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = TRUE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Bottom 20 Positive Genes Cystic P28", angle_col = 45)

#sort by variance
Post_cyst_P28_variation <- apply(conv_cts19, 1, var)
#top 20 most variable genes
top20_post_cyst_P28_variation <- names(sort(Post_cyst_P28_variation, decreasing = TRUE))[1:20]

 #20 most varied genes
#estimate variance for each row 
pheatmap(conv_cts19[top20_post_cyst_P28_variation,], cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=TRUE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "20 Most Varied Genes Cystic P28", angle_col = 45) #, annotation_col=meta_39$Genotype)
```

#volcano plots
```{r}
#Recall EnhancedVolcano
library(EnhancedVolcano)
#create dataframe of data for volcano plot
Vol_cystic_P28 <- as.data.frame(res_19)
#ENMUSG to columns
TopT_cystic_P28 <- tibble:::rownames_to_column(Vol_cystic_P28)
#Rename Columns
names(TopT_cystic_P28)[1] <- "gene_id"
#Merge Gene symbols into dataframe
TopT_finish_cystic_P28 <- left_join(TopT_cystic_P28, conversion2, by = "gene_id")
#Omit all NA's
TopT_finish_cystic_P28 <- na.omit(TopT_finish_cystic_P28)
#remove unneeded columns
TopT_finish_cystic_P28 <- TopT_finish_cystic_P28[,!(colnames(TopT_finish_cystic_P28) %in% c("gene_id", "baseMean", "lfcSE", "stat"))]
#remove repeating data
TopT_finish_cystic_P28 <- dplyr::distinct(TopT_finish_cystic_P28, gene_name, .keep_all = TRUE)
#gene symbols to rownames
TopT_finish_cystic_P28 <- tibble:::column_to_rownames(TopT_finish_cystic_P28, "gene_name")

View(TopT_finish_cystic_P28)

#Volcano Plot w/ strict pvalue cutoff
EnhancedVolcano(TopT_finish_cystic_P28, lab = rownames(TopT_finish_cystic_P28), x = 'log2FoldChange', y = 'pvalue', pCutoff = 10e-32, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0, col = c("#440154", "#21918c", "#5ec962", "#fde725"))
#Volcano plot Cystic P28 w/ viridis colors and normal pvalue cutoff
...
```
#Check for drug targets
```{r}
#genes with LFC>2 from results
...
#rename columns to reflect drug and target
...
target_gene_overlap_DESEQ2_19 <- merge(deg_19_up, gene_targets_unique, by = "gene_name")
View(target_gene_overlap_DESEQ2_19)
#Save file
write.csv(target_gene_overlap_DESEQ2_19, file = "C:\\Users\\mattr\\Documents\\Lasseigne_Lab_KURE\\TFEA_Lasseigne_Lab_Brownlee\\target_gene_overlap_DESEQ2_19.csv")
...
Limma_target_gene_overlap_DESEQ2_19 <- merge(deg_19_up, Limma_Drug_Targets_unique, by = "gene_name")
View(Limma_target_gene_overlap_DESEQ2_19)
write.csv(Limma_target_gene_overlap_DESEQ2_19, file = "C:\\Users\\mattr\\Documents\\Lasseigne_Lab_KURE\\TFEA_Lasseigne_Lab_Brownlee\\Limma_target_gene_overlap_DESEQ2_19.csv")
```
...
rhub_target_gene_overlap_DESEQ2_19 <- merge(deg_19_up, Rhub_Launched_Drug_Targets_Unique, by = "gene_name")
View(rhub_target_gene_overlap_DESEQ2_19)
#Save file
write.csv(rhub_target_gene_overlap_DESEQ2_19, file = "C:\\Users\\mattr\\Documents\\Lasseigne_Lab_KURE\\TFEA_Lasseigne_Lab_Brownlee\\Rhub_target_gene_overlap_DESEQ2_19.csv")
```
