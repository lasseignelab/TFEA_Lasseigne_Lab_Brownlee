...
```{r}
#test w/ viridis
df<- as.data.frame(rld_nf)
plotPCA(rld_nf, intgroup=c("Genotype")) + geom_label_repel(aes(rld_nf@colData@rownames), box.padding   = 0.15, label.padding = 0.15, point.padding  = 0.3, segment.color = "grey50", gridlines.major = FALSE, gridlines.minor = FALSE)

#original
plotPCA(rld_nf, intgroup=c("Genotype")) + geom_label_repel(aes(label = rld_nf@colData@rownames), box.padding   = 0.15, label.padding = 0.15, point.padding = 0.3, segment.color = 'grey50', gridlines.major = FALSE, gridlines.minor = FALSE)
```
