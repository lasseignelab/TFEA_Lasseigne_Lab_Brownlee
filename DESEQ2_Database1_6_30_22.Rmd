...
pheatmap(conv_cts_fixed[fixedtop20, ], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = TRUE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "Top 20 Positive LFC", angle_col = 45)
...
#create heatmap
pheatmap(conv_cts[top,], cluster_rows=TRUE, cluster_cols=TRUE, show_rownames=TRUE, color = viridis(n= 10000, alpha = 1, begin = 0, end = 1, option = "viridis"), border_color = "NA", main = "20 Most Varied LFC Genes", angle_col = 45) #, annotation_col=meta_39$Genotype)
...
gene_targets_unique <- unique(gene_targets$Target)
count_test <- plotCounts(dds_39, gene = which.min(res_nf$padj), intgroup = "Genotype", returnData = TRUE)
ggplot(count_test, aes(x = Genotype, y = count)) + geom_point(position = position_jitter(w=0.1,h=0)) + scale_y_log10(breaks = c(25,100,400))
```
